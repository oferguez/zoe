...
export function CommunityPostCard({
  ...
  return (
    <article className="rounded-3xl border border-cornflower-blue/12 bg-white/90 p-6 shadow-[0_18px_55px_rgba(40,45,69,0.06)] md:p-8">
      ...
      <h2 className="mb-5 text-2xl font-black leading-tight text-ink">{post.title}</h2>
      ...
      <div className="mb-6 grid gap-5 border-b border-ink/10 pb-6 md:grid-cols-3">
        ...
        <section>
          ...
          <p className="text-sm font-semibold leading-6 text-ink/75">{post.durationLabel}</p>
        </section>
        <section>
      ...
      <div className="grid gap-3">
        {post.comments.map((comment) => (
          <section
            className="rounded-2xl border border-cornflower-blue/12 bg-linen-white/45 p-4"
            key={comment.id}
          >
